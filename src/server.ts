...
// 1. Booking Schema (Extending Inquiry)
interface IBooking {
  _id?: string;
  id?: string;
  name: string;
...
function getLocalBookings(): IBooking[] {
  try {
...
function saveLocalBooking(booking: IBooking) {
  try {
...
function updateLocalBookingStatus(id: string, status: "accepted" | "rejected"): boolean {
  try {
    const bookings = getLocalBookings();
...
function saveLocalServices(services: IService[]) {
  try {
...
function getLocalGallery(): IGallery[] {
  try {
...
function saveLocalGallery(gallery: IGallery[]) {
  try {
...
app.get("/api/inquiries", async (req, res) => {
  try {
    if (isMongoConnected) {
      ...
      const bookings = getLocalBookings().sort(
...
app.post("/api/services", async (req, res) => {
  ...
  const newService: IService = { icon, title, desc, img, createdAt: new Date() };
...
app.delete("/api/services/:id", async (req, res) => {
  ...
  try {
    if (isMongoConnected) {
      ...
      if (!deleted) return res.status(404).json({ error: "Service card not found" });
...
app.post("/api/gallery", async (req, res) => {
  ...
  const newGallery: IGallery = { src, cat, tall: !!tall, alt, createdAt: new Date() };
...
app.delete("/api/gallery/:id", async (req, res) => {
  ...
  try {
    if (isMongoConnected) {
      ...
      if (!deleted) return res.status(404).json({ error: "Gallery card not found" });
